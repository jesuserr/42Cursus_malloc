...
/*   Updated: 2024/10/26 20:28:39 by jesuserr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// of the system page size, just in case, it is done explicitly. Received size
// is the requested user size + bytes needed for memory alignment.
...
// In some literature it is said that mmap rounds up 'size' to the next multiple
// of the system page size, just in case, it is done explicitly. Expands LARGE
// heap creating a new block and attaching it to the end of the linked list of
// LARGE blocks.
...
void	*malloc(size_t size)
{
	...
	if (!g_heaps[LARGE_HEAP])
		ptr = init_large_heap(size);
	else
		ptr = add_block_to_large_heap(size);
	if (ptr)
